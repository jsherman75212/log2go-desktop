...
test('desktop backend client logs in with username password and optional device id', async () => {
  const restore = installFetch((url, init) => {
    ...
    return Response.json({ access_token: 'token-1', token_type: 'bearer', user_id: 7, device_id: 'desktop-device-1' });
  });
...
test('desktop backend client sends bearer token for account-wide app data', async () => {
  ...
  try {
    const profile = await getAccountProfile('https://api.example.test', 'token-1');
    const contacts = await listContacts('https://api.example.test', 'token-1');
    assert.equal(profile.callsign, 'KE5ZQV');
    assert.deepEqual(contacts, []);
    assert.deepEqual(calls, [
      'GET https://api.example.test/api/v1/account Bearer token-1',
      'GET https://api.example.test/api/v1/contacts Bearer token-1',
    ]);
  } finally {
    restore();
  }
...
test('desktop backend client loads and saves station profiles through account endpoint', async () => {
  ...
  try {
    const loaded = await getStationProfiles('https://api.example.test', 'token-1');
    const saved = await saveStationProfiles('https://api.example.test', 'token-1', collection);
    assert.equal(loaded.activeProfileId, 'truck-mobile');
    assert.equal(saved.profiles[0].callsign, 'KE5ZQV');
    assert.deepEqual(calls, [
      'GET https://api.example.test/api/v1/account/station-profiles Bearer token-1',
      'PUT https://api.example.test/api/v1/account/station-profiles Bearer token-1',
    ]);
  } finally {
    restore();
  }
